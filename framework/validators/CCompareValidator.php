...
class CCompareValidator extends CValidator
{
	...
	protected function validateAttribute($object,$attribute)
	{
		...
		switch($this->operator)
		{
			...
			case '==':
				...
				{
					$message=$this->message!==null?$this->message:Yii::t('yii','{attribute} debe ser exactamente igual.');
			...
			case '!=':
				if(($this->strict && $value===$compareValue) || (!$this->strict && $value==$compareValue))
				{
					$message=$this->message!==null?$this->message:Yii::t('yii','{attribute} no debe ser igual a "{compareValue}".');
			...
			case '>':
				...
				{
					$message=$this->message!==null?$this->message:Yii::t('yii','{attribute} debe ser mayor que "{compareValue}".');
					$this->addError($object,$attribute,$message,array('{compareAttribute}'=>$compareTo,'{compareValue}'=>$compareValue));
				}
				break;
			case '>=':
				if($value<$compareValue)
				{
					$message=$this->message!==null?$this->message:Yii::t('yii','{attribute} debe ser mayor o igual que "{compareValue}".');
					$this->addError($object,$attribute,$message,array('{compareAttribute}'=>$compareTo,'{compareValue}'=>$compareValue));
				}
				break;
			case '<':
				if($value>=$compareValue)
				{
					$message=$this->message!==null?$this->message:Yii::t('yii','{attribute} debe ser menor que "{compareValue}".');
					$this->addError($object,$attribute,$message,array('{compareAttribute}'=>$compareTo,'{compareValue}'=>$compareValue));
				}
				break;
			case '<=':
				if($value>$compareValue)
				{
					$message=$this->message!==null?$this->message:Yii::t('yii','{attribute} debe ser menor o igual que "{compareValue}".');
					$this->addError($object,$attribute,$message,array('{compareAttribute}'=>$compareTo,'{compareValue}'=>$compareValue));
				}
				break;
			default:
				throw new CException(Yii::t('yii','Operador no v&aacute;lido "{operator}".',array('{operator}'=>$this->operator)));
		}
	}
	...
	public function clientValidateAttribute($object,$attribute)
	{
		...
		switch($this->operator)
		{
			...
			case '==':
				if($message===null)
					$message=Yii::t('yii','{attribute} debe ser exactamente igual.');
				$condition='value!='.$compareValue;
				break;
			case '!=':
				if($message===null)
					$message=Yii::t('yii','{attribute} no debe ser igual que "{compareValue}".');
				$condition='value=='.$compareValue;
				break;
			case '>':
				if($message===null)
					$message=Yii::t('yii','{attribute} debe ser mayor que "{compareValue}".');
				$condition='parseFloat(value)<=parseFloat('.$compareValue.')';
				break;
			case '>=':
				if($message===null)
					$message=Yii::t('yii','{attribute} debe ser mayor o igual que "{compareValue}".');
				$condition='parseFloat(value)<parseFloat('.$compareValue.')';
				break;
			case '<':
				if($message===null)
					$message=Yii::t('yii','{attribute} debe ser menor que "{compareValue}".');
				$condition='parseFloat(value)>=parseFloat('.$compareValue.')';
				break;
			case '<=':
				if($message===null)
					$message=Yii::t('yii','{attribute} debe ser menor o igual que "{compareValue}".');
				$condition='parseFloat(value)>parseFloat('.$compareValue.')';
				break;
			default:
				throw new CException(Yii::t('yii','Operador no v&aacute;lido "{operator}".',array('{operator}'=>$this->operator)));
