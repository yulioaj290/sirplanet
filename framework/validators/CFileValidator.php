...
class CFileValidator extends CValidator
{
	...
	protected function validateAttribute($object, $attribute)
	{
		if($this->maxFiles > 1)
		{
			...
			{
				$message=$this->tooMany!==null?$this->tooMany : Yii::t('yii', '{attribute} no se aceptan m&aacute;s de {limit} archivos.');
	...
	protected function validateFile($object, $attribute, $file)
	{
		...
		{
			$message=$this->tooLarge!==null?$this->tooLarge : Yii::t('yii','El archivo "{file}" es muy grande. No se puede exeder de {limit} bytes.');
			$this->addError($object,$attribute,$message,array('{file}'=>$file->getName(), '{limit}'=>$this->getSizeLimit()));
		}
		else if($error==UPLOAD_ERR_PARTIAL)
			throw new CException(Yii::t('yii','El archivo "{file}" ha sido subido parcialmente.',array('{file}'=>$file->getName())));
		else if($error==UPLOAD_ERR_NO_TMP_DIR)
			throw new CException(Yii::t('yii','Designe la carpeta temporal para almacenar los archivos subidos "{file}".',array('{file}'=>$file->getName())));
		else if($error==UPLOAD_ERR_CANT_WRITE)
			throw new CException(Yii::t('yii','Fallo a la hora de escribir el archivo "{file}" en el disco.',array('{file}'=>$file->getName())));
		...
		{
			$message=$this->tooSmall!==null?$this->tooSmall : Yii::t('yii','El archivo "{file}" es muy peque&ntilde;o. No puede ser menor de {limit} bytes.');
		...
		if($this->types!==null)
		{
			...
			{
				$message=$this->wrongType!==null?$this->wrongType : Yii::t('yii','El archivo "{file}" no puede ser subido. Solo se pueden subir archivos con estas extensiones: {extensions}.');
	...
	protected function emptyAttribute($object, $attribute)
	{
		if(!$this->allowEmpty)
		{
			$message=$this->message!==null?$this->message : Yii::t('yii','{attribute} no puede quedar vac&iacute;o.');
