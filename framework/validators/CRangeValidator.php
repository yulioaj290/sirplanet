...
class CRangeValidator extends CValidator
{
	...
	protected function validateAttribute($object,$attribute)
	{
		...
		if(!is_array($this->range))
			throw new CException(Yii::t('yii','La propiedad del "rango" debe ser especificada con una lista de valores.'));
		if(!$this->not && !in_array($value,$this->range,$this->strict))
		{
			$message=$this->message!==null?$this->message:Yii::t('yii','{attribute} no est&aacute; en la lista.');
			$this->addError($object,$attribute,$message);
		}
		else if($this->not && in_array($value,$this->range,$this->strict))
		{
			$message=$this->message!==null?$this->message:Yii::t('yii','{attribute} est&aacute; en la lista.');
			$this->addError($object,$attribute,$message);
		}
	}
	...
	public function clientValidateAttribute($object,$attribute)
	{
		...
		if(($message=$this->message)===null)
			$message=$this->not ? Yii::t('yii','{attribute} est&aacute; en la lista.') : Yii::t('yii','{attribute} no est&aacute; en la lista.');
